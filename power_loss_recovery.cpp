 ...
 */
void check_print_job_recovery() {//查看是否有断电续打的文件
  ...
  if (card.cardOK) {
    ...
    if (card.jobRecoverFileExists()) { //如果有断电续打的文件，
      card.openJobRecoveryFile(true);   //打开文件
      card.loadJobRecoveryInfo();       //下载信息
      card.closeJobRecoveryFile();      //关闭文件
